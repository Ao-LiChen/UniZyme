...
ClipZymeFeature=torch.load("../data/Data_Split/ClipZyme_Feature.pt")
...
def my_collate_fn(batch):
    ...
    # If there are no valid samples in this batch, you can either
    # return None or raise an exception. Often returning None means
    # skipping the batch. Another approach might be to produce an
    # "empty" valid batch.
    if len(batch) == 0:
        return None

    # Otherwise, return the collated batch the usual way
...
class SequenceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            mernum_clip_feature = ClipZymeFeature[mernum_name]
        except:
            return None
        ...
        mernum_clip_feature.requires_grad_(False)

        return (mernum_clip_feature,batch_tokens_uniprot,batch_tokens_enzyme,batch_lens_uniprot,batch_lens_enzyme,Enzymes_distances,Substrate_distances,Enzymes_energy,Enzymes_mask,Substrate_mask,activate_labels_batch,cleavage_labels_batch)
...
class Transformer_DE(nn.Module):
    def __init__(self, n_layers=1, d_model=480, n_heads=2,K=10, device="cuda"):
        ...
        self.cliptransfer=nn.Linear(1280,480)

    #def forward(self, input_feature, distance_matrix, energy_matrix, mask):
    def forward(self, mernum_clip_feature,Substrate_features, Substrate_distances, Substrate_mask, Enzymes_features, Enzymes_distances, Enzymes_energy, Enzymes_mask):

        # for layer in self.layers:
        #     Enzymes_features = layer(Enzymes_features, Enzymes_distances, Enzymes_energy, Enzymes_mask)

        Enzymes_features = self.cliptransfer(mernum_clip_feature)
...
train_loader = DataLoader(train_data, batch_size=real_batch_size, shuffle=True, num_workers=8, collate_fn=my_collate_fn)
test_loader = DataLoader(test_data, batch_size=real_batch_size, shuffle=False, num_workers=8, collate_fn=my_collate_fn)
...
for epoch in range(num_epochs):
    ...
    for i, (mernum_clip_feature,batch_tokens_uniprot,batch_tokens_enzyme,batch_lens_uniprot,batch_lens_enzyme,Enzymes_distances,Substrate_distances,Enzymes_energy,Enzymes_mask,Substrate_mask,activate_labels_batch,cleavage_labels_batch) in enumerate(train_loader):
        ...
        Activate_Site, Cleavage_Site = Transformer_DE(mernum_clip_feature,Substrate_features, Substrate_distances, Substrate_mask, Enzymes_features, Enzymes_distances, Enzymes_energy, Enzymes_mask)
        #Activate_Site, Cleavage_Site = Transformer_DE(esm_features_batch, distances_batch, energy_batch, mask)
        ...
        #print(loss.item())
        print("train_loss", loss.item(), "train_loss_cleavage", loss_cleavage.item())
    ...
    Transformer_DE.eval()
    with torch.no_grad():
        for i, (mernum_clip_feature,batch_tokens_uniprot, batch_tokens_enzyme, batch_lens_uniprot, batch_lens_enzyme, Enzymes_distances,
            ...
            Activate_Site, Cleavage_Site = Transformer_DE(mernum_clip_feature,Substrate_features, Substrate_distances, Substrate_mask,
            ...
            test_loss += loss.item()
            #print loss
            print("test_loss",loss.item(),"test_loss_cleavage",loss_cleavage.item())
